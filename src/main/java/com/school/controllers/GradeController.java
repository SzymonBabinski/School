...
public class GradeController {
  ...
  public ResponseEntity<GradeDto> createGradeToStudent(@PathVariable("studentId") int id,
  ...
  public ResponseEntity<List<GradeDto>> getGrades(@PathVariable("studentId") int id) {
    ...
    return studentService.findStudentById(id).map(student -> ResponseEntity.ok(gradeMapper.mapToGradeDto(student.getGrades()))).orElse(ResponseEntity.notFound().build());
  }

  @GetMapping("/students/{studentId}/grades/{gradeId}")
  public ResponseEntity<GradeDto> gradeInfo(@PathVariable("studentId") int studentId,
                                            @PathVariable("gradeId") int gradeId) {

    // Nie mam pomyslu na to ale i tak jest chyba lepiej niz bylo
    if (studentService.findStudentById(studentId).isPresent() && gradeService.findGrade(gradeId).isPresent()) {
      return ResponseEntity.ok().body(gradeMapper.mapToGradeDto(gradeService.showInfoAboutStudentGrade(studentId,
              gradeId)));
    } else {
  ...
  public ResponseEntity<GradeDto> deleteGrade(@PathVariable("studentId") int studentId,
                                              @PathVariable("gradeId") int gradeId) {
